...
<body class="bg-gray-100">
    ...
    <!-- Navbar -->
    <header class="bg-white shadow-sm px-4 md:px-8 py-4 flex justify-between items-center" x-data="{ open: false }">
        <div class="flex items-center space-x-4 md:space-x-14 w-full justify-between md:justify-start">
            <!-- Logo -->
            <img src="/logo.png" class="w-20 h-12 md:w-28 md:h-16" alt="logo">
            ...
            <nav class="hidden md:flex items-center space-x-8">
                @if (auth()->user()->role === 'admin')
                    <a href="{{ route('admin.dashboard') }}"
                        class="font-semibold border-b-2 text-xs md:text-sm {{ request()->routeIs('admin.dashboard') ? 'text-green-700 border-green-700' : 'text-gray-600 border-transparent hover:text-green-600 hover:border-green-600' }}">
                        Dashboard
                    </a>
                    <a href="{{ route('admin.membership') }}"
                        class="font-semibold border-b-2 text-xs md:text-sm {{ request()->routeIs('admin.membership') ? 'text-green-700 border-green-700' : 'text-gray-600 border-transparent hover:text-green-600 hover:border-green-600' }}">
                        Old Memberships
                    </a>
                    <a href="{{ route('admin.user') }}"
                        class="font-semibold border-b-2 text-xs md:text-sm {{ request()->routeIs('admin.user') ? 'text-green-700 border-green-700' : 'text-gray-600 border-transparent hover:text-green-600 hover:border-green-600' }}">
                        Non Memberships
                    </a>
                    <a href="{{ route('admin.newMembership') }}"
                        class="font-semibold border-b-2 text-xs md:text-sm {{ request()->routeIs('admin.newMembership') ? 'text-green-700 border-green-700' : 'text-gray-600 border-transparent hover:text-green-600 hover:border-green-600' }}">
                        New Memberships
                    </a>
                    <a href="#"
                        class="font-semibold border-b-2 text-xs md:text-sm {{ request()->routeIs('') ? 'text-green-700 border-green-700' : 'text-gray-600 border-transparent hover:text-green-600 hover:border-green-600' }}">
                        Events
                    </a>
                @elseif(auth()->user()->role === 'user')
                    <a href="{{ route('profile') }}" class="font-bold hover:text-green-300 text-xs md:text-sm">
                        My Profile
                    </a>
                @endif
            </nav>
        ...
        <div class="hidden md:flex items-center space-x-4">
            ...
            <span class="w-32 text-gray-600 text-xs md:text-sm">{{ Auth::user()->name ?? 'Admin' }}</span>
        </div>

        <!-- Mobile Dropdown Menu -->
        <div x-show="open" @click.away="open = false"
            class="absolute top-16 left-0 w-full bg-white shadow-md md:hidden flex flex-col space-y-2 px-8 py-4">
                ...
                @if (auth()->user()->role === 'admin')
                    ...
                    <div x-data="{ open: false }" class="relative">
                        ...
                        <div x-show="open" @click.away="open = false"
                            ...
                            <a href="{{ route('admin.newMembership') }}"
                                class="px-4 py-2 text-xs md:text-sm text-gray-700 hover:bg-green-100 hover:text-green-700 {{ request()->routeIs('admin.newMembership') ? 'font-semibold text-green-700' : '' }}">
                                New Membership
                            </a>
                            <a href="{{ route('admin.membership') }}"
                                class="px-4 py-2 text-xs md:text-sm text-gray-700 hover:bg-green-100 hover:text-green-700 {{ request()->routeIs('admin.membership') ? 'font-semibold text-green-700' : '' }}">
                                Old Membership
                            </a>
                            <a href="{{ route('admin.user') }}"
                                class="px-4 py-2 text-xs md:text-sm text-gray-700 hover:bg-green-100 hover:text-green-700 {{ request()->routeIs('admin.user') ? 'font-semibold text-green-700' : '' }}">
                                Non Membership
                            </a>
                        </div>
                    </div>
