...
<div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <h1 class="text-3xl font-semibold mb-6 text-gray-900">No Memberships</h1>
            ...
            <tbody>
                @foreach ($memberships as $membership)
                    ...
                    <tr class="hover:bg-green-50 transition-colors duration-200">
                        ...
                        <td class="px-12 py-4 text-sm text-gray-900 border-b border-gray-200">{{ $membership->user->name }}</td>
                        <td class="px-24 py-4 text-sm text-gray-700 border-b border-gray-200">{{ $membership->user->ngo }}</td>
                        <td class="px-12 py-4 text-sm text-gray-700 border-b border-gray-200 break-words max-w-xs">{{ $membership->user->email }}</td>
                        <td class="px-12 py-4 text-sm text-gray-500 border-b border-gray-200">{{ $membership->created_at->format('Y-m-d H:i') }}</td>
