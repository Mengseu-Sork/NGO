...
<body class="bg-green-600 font-sans">
  ...
  <div class="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-md">
    ...
    <form action="{{ route('membership.submitUpload') }}" method="POST" enctype="multipart/form-data" novalidate>
      ...
      {{-- Additional Info --}}
      <h3 class="text-lg font-semibold text-green-700 mb-3"><i class="fas fa-info-circle mr-2"></i> Additional Information about Your Organization</h3>
      ...
      <input type="text" id="mailing-address" name="mailing-address" value="{{ old('mailing-address') }}"
             class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-green-400"  placeholder="Enter Mailing Address"
      >
      @error('mailing-address') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
      ...
      <input type="text" id="physical-address" name="physical-address" value="{{ old('physical-address') }}"
             class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-green-400" placeholder="Enter Physical Address"
      >
      @error('physical-address') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
      ...
      <input type="text" id="facebook" name="facebook" value="{{ old('facebook') }}"
             class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-green-400" placeholder="Enter Facebook URL"
      >
      ...
      <input type="text" id="website" name="website" value="{{ old('website') }}"
             class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-green-400" placeholder="Enter Website URL"
      >
      ...
      <div class="space-y-4">
        ...
        <div>
          <label class="inline-flex items-center space-x-2 cursor-pointer">
            <input type="checkbox" id="comm-telegram" name="comm-channel[]" value="Telegram" onchange="togglePhoneInput('telegram')"
              class="form-checkbox h-5 w-5 text-green-600"
              {{ in_array('Telegram', $commChannelsOld) ? 'checked' : '' }}>
            <span>Telegram</span>
          </label>
          <div id="phone-group-telegram" class="mt-2" style="display: {{ in_array('Telegram', $commChannelsOld) ? 'block' : 'none' }};">
            <label for="phone-telegram" class="block font-normal mb-1">Telegram Phone Number <span class="text-red-600">*</span></label>
            <input type="text" id="phone-telegram" name="phone-telegram" placeholder="Telegram Phone Number" value="{{ old('phone-telegram') }}"
                   class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-green-400"
            >
            @error('phone-telegram') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
          </div>
        </div>
        ...
        <div>
          <label class="inline-flex items-center space-x-2 cursor-pointer">
            <input type="checkbox" id="comm-signal" name="comm-channel[]" value="Signal" onchange="togglePhoneInput('signal')"
              class="form-checkbox h-5 w-5 text-green-600"
              {{ in_array('Signal', $commChannelsOld) ? 'checked' : '' }}>
            <span>Signal</span>
          </label>
          <div id="phone-group-signal" class="mt-2" style="display: {{ in_array('Signal', $commChannelsOld) ? 'block' : 'none' }};">
            <label for="phone-signal" class="block font-normal mb-1">Signal Phone Number <span class="text-red-600">*</span></label>
            <input type="text" id="phone-signal" name="phone-signal" placeholder="Signal Phone Number" value="{{ old('phone-signal') }}"
                   class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-green-400"
            >
            @error('phone-signal') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
          </div>
        </div>
        ...
        <div>
          <label class="inline-flex items-center space-x-2 cursor-pointer">
            <input type="checkbox" id="comm-whatsapp" name="comm-channel[]" value="WhatsApp" onchange="togglePhoneInput('whatsapp')"
              class="form-checkbox h-5 w-5 text-green-600"
              {{ in_array('WhatsApp', $commChannelsOld) ? 'checked' : '' }}>
            <span>WhatsApp</span>
          </label>
          <div id="phone-group-whatsapp" class="mt-2" style="display: {{ in_array('WhatsApp', $commChannelsOld) ? 'block' : 'none' }};">
            <label for="phone-whatsapp" class="block font-normal mb-1">WhatsApp Phone Number <span class="text-red-600">*</span></label>
            <input type="text" id="phone-whatsapp" name="phone-whatsapp" placeholder="WhatsApp Phone Number" value="{{ old('phone-whatsapp') }}"
                   class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-green-400"
            >
            @error('phone-whatsapp') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
          </div>
        </div>
      ...
      @foreach([
        ...
        <div class="mb-6 border border-gray-300 rounded-md p-4">
          <label for="{{ $field }}" class="block font-normal mb-3">
            {{ $label }}
            @if($field !== 'fundraising-strategy') <span class="text-red-600">*</span> @endif
          </label>
          <input type="file" id="{{ $field }}" name="{{ $field }}" @if($field !== 'fundraising-strategy') required @endif
      ...
      {{-- Textareas --}}
      <label for="vision" class="block font-semibold mb-2">Vision Statement <span class="text-red-600">*</span></label>
      ...
      @error('vision') <p class="text-red-600 text-sm mb-4">{{ $message }}</p> @enderror

      <label for="mission" class="block font-semibold mb-2">Mission Statement <span class="text-red-600">*</span></label>
      ...
      @error('mission') <p class="text-red-600 text-sm mb-4">{{ $message }}</p> @enderror

      <label for="goal" class="block font-semibold mb-2">Goal Statement <span class="text-red-600">*</span></label>
      ...
      @error('goal') <p class="text-red-600 text-sm mb-4">{{ $message }}</p> @enderror
      ...
      @error('objectives') <p class="text-red-600 text-sm mb-4">{{ $message }}</p> @enderror

      <div class="bg-green-100 p-4 rounded-md mb-6 text-green-900">
        <strong class="block mb-2">Pledge of Commitment</strong>
        <p class="text-sm">
          On behalf of my organization, I accept the Mission statement and Values of the NGO Forum on Cambodia and agree to abide by the By-Laws governing membership. I believe that my organization fully qualifies for membership, as outlined in the NGO Forum's By-Laws, and have attached the required documentary evidence.<br>
          My organization agrees to participate regularly in member meetings and to pay annual membership fees (except where exempt).
        </p>
      </div>

      <label for="director-name" class="block font-semibold mb-2">Name of Director <span class="text-red-600">*</span></label>
      <input type="text" id="director-name" name="director-name" value="{{ old('director-name') }}" required placeholder="Enter Your Name"
             class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-green-400 mb-4"
      >
      @error('director-name') <p class="text-red-600 text-sm mb-4">{{ $message }}</p> @enderror
      ...
      <input type="text" id="title" name="title" value="{{ old('title') }}" required placeholder="Enter Title"
             class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-green-400 mb-4"
      >
      @error('title') <p class="text-red-600 text-sm mb-4">{{ $message }}</p> @enderror
      ...
      >
      @error('date') <p class="text-red-600 text-sm mb-4">{{ $message }}</p> @enderror
